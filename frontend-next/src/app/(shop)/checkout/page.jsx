...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    const fetchMethods = async () => {
      try {
        const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/delivery_methods');
        ...
        <div style={{ maxWidth: '600px', width: '100%' }}>
          ...
          <form onSubmit={handlePlaceOrder} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                <input 
                  ...
                  onBlur={() => {
                    if (phone && /^(?:\+88|88)?01[3-9]\d{8}$/.test(phone)) {
                      fetch(process.env.NEXT_PUBLIC_API_URL + '/api/abandoned-carts/track', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                          phone,
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '1rem' }}>
              <Link href="/cart" style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', color: 'var(--accent-primary)', textDecoration: 'none', fontWeight: 500 }}>
