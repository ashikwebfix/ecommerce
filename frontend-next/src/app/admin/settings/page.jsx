...
const AdminSettings = () => {
  ...
  const fetchSettings = async () => {
    try {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/delivery_methods', { headers: { Authorization: `Bearer ${token}` }});
      const data = await res.json();
      setDeliveryMethods(data);

      const menuRes = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/header_menu', { headers: { Authorization: `Bearer ${token}` }});
      const menuData = await menuRes.json();
      setHeaderMenu(menuData);

      const trackingRes = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/tracking_settings', { headers: { Authorization: `Bearer ${token}` }});
      if (trackingRes.ok) {
        const trackingData = await trackingRes.json();
        if (trackingData) setTrackingSettings(trackingData);
      }

      const storefrontRes = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/storefront_ui', { headers: { Authorization: `Bearer ${token}` }});
      if (storefrontRes.ok) {
        const storefrontData = await storefrontRes.json();
        if (storefrontData) setStorefrontUI(storefrontData);
      }

      const pathaoRes = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/pathao_settings', { headers: { Authorization: `Bearer ${token}` }});
      if (pathaoRes.ok) {
        const pathaoData = await pathaoRes.json();
        if (pathaoData) setPathaoSettings(pathaoData);
      }

      const generalRes = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/general_settings', { headers: { Authorization: `Bearer ${token}` }});
      if (generalRes.ok) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res1 = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/delivery_methods', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ value: deliveryMethods })
      });
      const res2 = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/header_menu', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ value: headerMenu })
      });
      const res3 = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/tracking_settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ value: trackingSettings })
      });
      const res4 = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/storefront_ui', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ value: storefrontUI })
      });
      const res5 = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/pathao_settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ value: pathaoSettings })
      });
      const res6 = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/general_settings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', overflowX: 'auto' }}>
        <button className={`tab-btn ${activeTab === 'general' ? 'active' : ''}`} onClick={() => router.push('/admin/settings?tab=general')}>General</button>
        <button className={`tab-btn ${activeTab === 'storefront' ? 'active' : ''}`} onClick={() => router.push('/admin/settings?tab=storefront')}>Storefront UI</button>
        <button className={`tab-btn ${activeTab === 'navigation' ? 'active' : ''}`} onClick={() => router.push('/admin/settings?tab=navigation')}>Navigation</button>
        <button className={`tab-btn ${activeTab === 'delivery' ? 'active' : ''}`} onClick={() => router.push('/admin/settings?tab=delivery')}>Delivery Rates</button>
        <button className={`tab-btn ${activeTab === 'tracking' ? 'active' : ''}`} onClick={() => router.push('/admin/settings?tab=tracking')}>Marketing</button>
        <button className={`tab-btn ${activeTab === 'courier' ? 'active' : ''}`} onClick={() => router.push('/admin/settings?tab=courier')}>Courier API</button>
      </div>
