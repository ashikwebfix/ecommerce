    ...
    const fetchStats = async () => {
      try {
        const [productsRes, usersRes, ordersRes] = await Promise.all([
          ...
          fetch(process.env.NEXT_PUBLIC_API_URL + '/api/users', { headers: { Authorization: `Bearer ${token}` } }),
          fetch(process.env.NEXT_PUBLIC_API_URL + '/api/orders', { headers: { Authorization: `Bearer ${token}` } })
        ]);
