...
const AdminProductForm = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const url = isEditing ? `${process.env.NEXT_PUBLIC_API_URL}/api/products/${id}` : `${process.env.NEXT_PUBLIC_API_URL}/api/products`;
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '2rem' }}>
      <header style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <button onClick={() => router.push('/admin/products')} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
