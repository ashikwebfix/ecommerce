...
const AdminBundles = () => {
  ...
  const fetchBundles = async () => {
    try {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/bundles', {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this bundle?')) {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bundles/${id}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
  ...
  return (
    <div className="animate-fade-in">
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <Link href="/admin/bundles/new" className="btn btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <tr key={bundle.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={{ padding: '1rem', textAlign: 'right' }}>
                  <div style={{ display: 'inline-flex', gap: '0.5rem' }}>
                    <Link href={`/admin/bundles/edit/${bundle.id}`} className="btn" style={{ padding: '0.5rem' }}>
                      <Edit size={16} />
                    </Link>
