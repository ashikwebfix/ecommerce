...
const AdminBundleForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = isEdit ? `${process.env.NEXT_PUBLIC_API_URL}/api/bundles/${id}` : process.env.NEXT_PUBLIC_API_URL + '/api/bundles';
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '4rem' }}>
      <header style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <button onClick={() => router.push('/admin/bundles')} className="btn-icon">
          <ArrowLeft size={20} />
        </button>
