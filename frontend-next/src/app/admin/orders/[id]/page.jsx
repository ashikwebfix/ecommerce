...
const AdminOrderDetails = () => {
  ...
  const openPathaoModal = async () => {
    ...
    try {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/pathao/cities', {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  useEffect(() => {
    if (selectedCity) {
      ...
      setSelectedZone('');
      setSelectedArea('');
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/pathao/zones/${selectedCity}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      .then(res => res.json())
  ...
  useEffect(() => {
    if (selectedZone) {
      setAreas([]);
      setSelectedArea('');
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/pathao/areas/${selectedZone}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      .then(res => res.json())
  ...
  const handleDispatchPathao = async () => {
    ...
    setPathaoLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/pathao/create-order/${id}`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
