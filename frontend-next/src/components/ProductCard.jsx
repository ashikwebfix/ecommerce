...
const ProductCard = ({ product, showRating }) => {
  ...
  return (
    ...
    >
      ...
      <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block', textDecoration: 'none', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', flex: 1, background: '#fff' }}>
        ...
        <Link href={`/product/${product.slug || product.id}`} style={{ textDecoration: 'none', color: 'inherit', marginBottom: 'auto' }}>
