...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/settings/header_menu');
        if (res.ok) {
        ...
        <div className="nav-desktop-menu" style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', borderRight: '1px solid var(--border-color)', paddingRight: '1.5rem' }}>
            {menuItems.map(item => {
              ...
              return (
                <Link key={item.id} href={item.url} className="nav-link" style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.95rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0rem', alignItems: 'center' }}>
            ...
            {userInfo && userInfo.token ? (
              <Link href="/profile" className="nav-link" style={{ padding: '0.5rem', display: 'flex', alignItems: 'center' }} title="Account">
                <Icons.User size={22} />
              </Link>
            ) : (
              <Link href="/login" className="nav-link" style={{ padding: '0.5rem', display: 'flex', alignItems: 'center' }} title="Login">
                <Icons.User size={22} />
              </Link>
            )}

            {/* Favorites */}
            <Link href={userInfo && userInfo.token ? "/profile" : "/login"} className="nav-link" style={{ padding: '0.5rem', display: 'flex', alignItems: 'center', position: 'relative' }} title="Favorites">
              <Icons.Heart size={22} />
              {favorites?.length > 0 && (
                <span style={{
            ...
            {/* Cart */}
            <Link href="/cart" className="nav-link" style={{ padding: '0.5rem', display: 'flex', alignItems: 'center', position: 'relative' }} title="Cart">
      ...
      <div className={`nav-mobile-menu ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {menuItems.map(item => {
              const IconComponent = item.icon ? Icons[item.icon] : null;
              return (
                <Link key={item.id} href={item.url} className="nav-link" onClick={() => setIsMobileMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around' }}>
            {userInfo && userInfo.token ? (
              <Link href="/profile" className="nav-link" onClick={() => setIsMobileMenuOpen(false)} title="Account">
                <Icons.User size={28} />
              </Link>
            ) : (
              <Link href="/login" className="nav-link" onClick={() => setIsMobileMenuOpen(false)} title="Login">
                <Icons.User size={28} />
              </Link>
            )}

            <Link href={userInfo && userInfo.token ? "/profile" : "/login"} className="nav-link" onClick={() => setIsMobileMenuOpen(false)} style={{ position: 'relative' }} title="Favorites">
              <Icons.Heart size={28} />
              {favorites?.length > 0 && (
                <span style={{
            ...
            <Link href="/cart" className="nav-link" onClick={() => setIsMobileMenuOpen(false)} style={{ position: 'relative' }} title="Cart">
